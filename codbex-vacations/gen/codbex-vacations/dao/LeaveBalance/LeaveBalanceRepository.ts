...
export class LeaveBalanceRepository {
    ...
    private async triggerEvent(data: LeaveBalanceEntityEvent | LeaveBalanceUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-vacations-LeaveBalance-LeaveBalance", ["trigger"]);
        ...
        producer.topic("codbex-vacations-LeaveBalance-LeaveBalance").send(JSON.stringify(data));
    }
}
