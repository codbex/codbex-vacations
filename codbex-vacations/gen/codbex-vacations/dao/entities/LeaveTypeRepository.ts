...
export class LeaveTypeRepository {
    ...
    private async triggerEvent(data: LeaveTypeEntityEvent | LeaveTypeUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-vacations-entities-LeaveType", ["trigger"]);
        ...
        producer.topic("codbex-vacations-entities-LeaveType").send(JSON.stringify(data));
    }
}
