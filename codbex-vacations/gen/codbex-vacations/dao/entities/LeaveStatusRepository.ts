...
export class LeaveStatusRepository {
    ...
    private async triggerEvent(data: LeaveStatusEntityEvent | LeaveStatusUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-vacations-entities-LeaveStatus", ["trigger"]);
        ...
        producer.topic("codbex-vacations-entities-LeaveStatus").send(JSON.stringify(data));
    }
}
