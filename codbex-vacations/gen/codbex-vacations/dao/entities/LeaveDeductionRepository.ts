...
export class LeaveDeductionRepository {
    ...
    private async triggerEvent(data: LeaveDeductionEntityEvent | LeaveDeductionUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-vacations-entities-LeaveDeduction", ["trigger"]);
        ...
        producer.topic("codbex-vacations-entities-LeaveDeduction").send(JSON.stringify(data));
    }
}
